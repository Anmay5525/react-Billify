...
describe("Testing CustomersList component if it", () => {
  ...
  it("renders without crashing", () => {
    ...
    ReactDOM.render(<CustomersList />, div);
  ...
  it("handles filter change", async () => {
    ...
    container
      .find(".filter-input")
      .simulate("change", { target: { value: "Da" } });
    expect(container.find("table").text().includes("John Stones")).toEqual(
      false
    );
  });
  ...
  it("contains button which turns filter off", async () => {
    ...
    expect(
      container
        ...
        .contains('<button type="button" class="simple-btn">Filter</button>')
    );
  });
});
